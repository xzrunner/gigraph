...
sm::vec3 Clamp(const sm::vec3& a, const sm::vec3& min, const sm::vec3& max)
{
    return sm::vec3(
...
void SplitBvh::BuildImpl(bbox const* bounds, int numbounds)
{
    ...
    std::vector<sm::vec3> centroids(numbounds);
...
SplitBvh::SahSplit SplitBvh::FindObjectSahSplit(SplitRequest const& req, PrimRefArray const& refs) const
{
    ...
    // in this case
    sm::vec3 centroid_extents = req.centroid_bounds.extents();
    if (centroid_extents.Dot(centroid_extents) == 0.f)
    {
        return split;
    }
...
SplitBvh::SahSplit SplitBvh::FindSpatialSahSplit(SplitRequest const& req, PrimRefArray const& refs) const
{
    ...
    // Extents
    sm::vec3 extents = req.bounds.extents();
    auto invarea = 1.f / req.bounds.surface_area();

    // If there are too few primitives don't split them
    if (extents.Dot(extents) == 0.f)
    {
        return split;
    }
    ...
    sm::vec3 origin = req.bounds.pmin;
    sm::vec3 binsize = req.bounds.extents() * (1.f / kNumBins);
    sm::vec3 invbinsize = sm::vec3(1.f / binsize.x, 1.f / binsize.y, 1.f / binsize.z);
    ...
    for (int i = req.startidx; i < req.startidx + req.numprims; ++i)
    {
        ...
        sm::vec3 firstbin = Clamp((primref.bounds.pmin - origin) * invbinsize, sm::vec3(0, 0, 0), sm::vec3(kNumBins - 1, kNumBins - 1, kNumBins - 1));
        // Determine finishing bin
        sm::vec3 lastbin = Clamp((primref.bounds.pmax - origin) * invbinsize, firstbin, sm::vec3(kNumBins - 1, kNumBins - 1, kNumBins - 1));
        // Iterate over axis
        for (int axis = 0; axis < 3; ++axis)
        {
